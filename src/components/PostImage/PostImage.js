...
export const PostImage = (props) => {
  const {
    src = 'https://cdn2.thecatapi.com/images/bcp.jpg',
  } = props;
  ...
  const onClickHandler = () => { // функция обработки нажатия на кнопку
  ...
  return (
    <ThemeContext.Consumer>
      {
        (value) => { // достаем значение темы из контекста, чтоб стилизовать кнопку
          return (
            <div className="image-wrapper">
              { !isHidden && <img src={src} className="img1"/> }
              <Button
                className={`${value === 'dark' ? 'dark' : 'light'} btn-sm btn-secondary`}
